...
int main (int argc, char** argv)
{
	...
	if(childpid1 == 0)
	{
		dup2(pipe1[0], 0); // replace the first child's stdin with
	...
	else
	{
		...
		if(childpid2 == 0)
		{
			dup2(pipe1[1], 1); // replace the second child's
					   // stdout with the write end of
					   // the first pipe
			dup2(pipe2[0], 0); // replace the second child's
					   // stdin with the read end of the
					   // second pipe
			close(pipe1[0]);   // close all pipes
			close(pipe1[1]);
			close(pipe2[0]);
		...
		else
		{
			...
			if(childpid3 == 0)
			{
				dup2(pipe2[1], 1); // replace the third child's
						   // stdout with the write end
						   // of the second pipe
				close(pipe1[0]);   // close all pipes
				close(pipe1[1]);
				close(pipe2[0]);
			...
			else
			{
				...
				close(pipe2[0]);
				close(pipe2[1]);
				system("sleep 1"); // parent sleeps while the
